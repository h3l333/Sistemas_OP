...
#define MAX_PAGS                                                               \
  1024 // Si la cantidad maxima de KB que puede almacenar la RAM es 1024 y todo
       // frame debe ser mayor o igual que 1KB, 1024 es un nro. apto
...
typedef struct Proceso {
  ...
  int pageTable[MAX_PAGS]; // inicializada en -1 antes de que se carguen paginas
                           // en RAM
} Proceso;

typedef struct Frame {
  int ocupado;      // 1 si esta ocupado y 0 si esta libre
  char procesoTipo; // s si esta asignado al SO y u si esta asignado a una
                    // pagina de un proceso de usuario
  int pid;
} Frame;
...
int cantProcActivos; // Contador cumple propósito de guardar la posición dentro
                     // del vector de procesos activos para agregar nuevos sin
                     // pisar procesos precedentes
int frameActual;
int cantFramesRAM;
...
void actualizarFrames(Frame tablaDeFrames[], Proceso *procesoNuevo,
                      int *frameActual) {
  int i = 0;
  while (i < (*frameActual))
    i++;
...
void actualizarTablaPags(Proceso *proceso, Frame tablaDeFrames[]) {
  ...
  // Una vez ubicado el frame base, inserto los indices en el array 'pageTable'
  // del proceso en cuestion uno por uno. j = indice de la tabla de pags.
  for (int j = 0; j < proceso->cantPags && i < cantFramesRAM &&
                  tablaDeFrames[i].pid == proceso->pid;
       j++) {
...
void inicializarTablaPags(
    Proceso *proceso) // funcion llamada al inicializarse el vector que guarda
...
void agregarProcesoAActivos(Proceso procesosActivos[MAX_PAGS],
                            Proceso procesoNuevo, int *cantProcActivos,
                            int *frameActual) {
  int i = 0;
...
void especificarParametros() {
  int ingresar;
  do {
    printf("Desea ingresar los parametros que definen el sistema "
           "computacional? (1: SI | 0: NO)\n");
    scanf("%d", &ingresar);
    if (ingresar == 1) {
      printf("Tamanio de la memoria real en KB:\n");
      scanf("%d", &tamanioRAMKBytes);
      while (tamanioRAMKBytes > MAX_RAM_KB) {
        printf("Ingreso invalido, vuelva a ingresar el tamanio de la memoria "
               "principal en KB:\n");
        scanf("%d", &tamanioRAMKBytes);
      }
      ...
      while (tamanioFrameKBytes > tamanioRAMKBytes || tamanioFrameKBytes <= 0) {
        printf("Ingreso invalido, vuelva a ingresar el tamanio de un frame en "
               "KB:\n");
        scanf("%d", &tamanioFrameKBytes);
      }
      ...
      while (cantFramesXProc > (tamanioRAMKBytes / tamanioFrameKBytes)) {
        printf("Ingreso invalido, vuelva a ingresar la cantidad de frames por "
               "proceso:\n");
        scanf("%d", &cantFramesXProc);
      }
      ...
      agregarProcesoAActivos(procesosActivos, procesoSO, &cantProcActivos,
                             &frameActual);
...
void cargarProceso(int *cantProcActivos, Proceso procesosActivos[MAX_PAGS],
                   Frame tablaDeFrames[], int *frameActual) {
  int ingresar;
  do {
    ...
    if (ingresar == 1) {
      ...
      if ((int)ceil(((double)procesoNuevo.tamanioKB / tamanioFrameKBytes)) >
          cantFramesXProc) {
        printf(
      ...
      procesoNuevo.cantPags =
          (int)ceil((double)procesoNuevo.tamanioKB / tamanioFrameKBytes);
      agregarProcesoAActivos(procesosActivos, procesoNuevo, cantProcActivos,
                             frameActual);
    } else if (ingresar != 0) {
      printf("Opcion invalida. Vuelva a intentar\n");
      printf("Desea ingresar los datos de un nuevo proceso? (1: SI | 0: NO)\n");
...
void imprimirTablaPags(int *procID, Proceso procesosActivos[],
                       int *cantProcActivos) {
...
int main() {
  ...
  printf("Tamanio RAM: %d, Tamanio SO: %d, Tamanio Frame: %d, Cant frames x "
         "proceso: %d\n",
         tamanioRAMKBytes, tamanioSOKBytes, tamanioFrameKBytes,
         cantFramesXProc);
  ...
  do {
    ...
    printf("5) Consultar la direccion fisica a partir de la virtual de un "
           "proceso\n");
    printf("0) Salir\n");
    scanf("%d", &operacion);
    switch (operacion) {
    case 1: {
      cargarProceso(&cantProcActivos, procesosActivos, tablaDeFrames,
                    &frameActual);
      break;
    }
    case 2: {
    ...
    case 3: {
      int procID;
      printf("Inserte el PID del proceso cuya tabla de paginas desea "
             "consultar:\n");
      scanf("%d", &procID);
    ...
    case 5: {
      ...
      printf("Indique el PID del proceso cuya direccion fisica quiere "
             "consultar: \n");
      scanf("%d", &pid);
      ...
      printf("La desea ingresar en formato binario o hexadecimal? (1: binario "
             "| 2: hex)\n");
      scanf("%d", &formato);
