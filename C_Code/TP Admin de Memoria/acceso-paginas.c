...
int hayFrameLibre(int tablaDeFrames[], int framesEnRAM) //Devuelvo indice libre si si, -1 si no
{
    for(int i = 0; i<framesEnRAM; i++)
    {
...
int devolverFrameVictima(int ultimoAcceso[], int tablaDeFrames[], int framesEnRAM)
{
...
void accesoPaginas(int stringReferencias[], int framesEnRAM)
{
    ...
    for(int i = 0; i<cantReferencias; i++)
    {
        ...
        if(estaEnRAM(stringReferencias[i], tablaDeFrames, framesEnRAM) == 0)
        {
            ...
            {
                //Hay frame libre
                tablaDeFrames[hayFrameLibre(tablaDeFrames, framesEnRAM)] = stringReferencias[i];
            }
            else
            {
                int frameVictima = devolverFrameVictima(ultimoAcceso, tablaDeFrames, framesEnRAM);
