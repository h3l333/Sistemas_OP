...
sem_t bufMutex, contVacio, contLleno; //sem_t es un typedef o estructura de datos que representa a un
    //semaforo.
//bufMutex representa a un semaforo que sirve el rol de asegurar que solo un hilo a la vez acceda a su
    //seccion critica.
//contVacio mantiene registro de la cantidad de espacios vacios en el buffer compartido
//contLleno mantiene registro de la cantidad de espacios llenos en el buffer compartido

int *buf; //puntero al buffer compartido
int bufPos = -1; //-1 indica que el buffer esta vacio. 0 que hay un objeto en el primer indice, 
    //1 que hay objetos hasta el segundo indice, etc.
int bufLargo, contProd, contConsumo; //indican el largo del buffer, cantidad de productores y
    //consumidores respectivamente

int produce(pthread_t id) //pasamos id del hilo para imprimir que hilo produce que numero
{
    int i = 0; //inicializamos el indice antes de buscar en el array de productores
    int p = 1 + rand()%100; //generamos un numero
    while(i < contProd && !pthread_equal(*(productores + i), id)) //escribiendo *(productores + i)
        //accedemos al valor en el indice i del array dinamico de productores y comparamos con el id
...
{
    int i = 0;
    while(i<contConsumo && !pthread_equal(*(consumidores + i), id)) //productores[i] == *(productores + i);
        i++; //buscamos indice correspondiente para imprimir que consumidor consumio que

    printf("Buffer: ");
    for(i = 0; i <= bufPos; i++)
        printf("%d", *(buf + i)); //imprimimos buffer antes de consumo
    printf("\nConsumidor %d consumio %d\nLargo actual del buffer: %d\n", i + 1, p, bufPos);
    //Indice del consumidor, nro. producido devuelto originalmente por produce(), posicion del 
    //buffer pre-consumo
}
...
int main(void)
{
	...
	for(i = 0; i < contProd; i++)
        ...
        //ID del hilo cuya por cuya ejecucion deseo esperar, valor de retorno de la funcion del hilo
        //Itero por los productores y espero a que cada uno termine
